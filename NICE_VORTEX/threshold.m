...
tfinish = 1000;	% Finish time (ps)     % reservoir steady state time needed: 100 ps; +20 ps;
tp = tfinish/2;
...
A0 = 0.2;
k0=0.5;
Inter = M + A0*exp(-1i*k0*(Y));

normal_sel = 0.6;

for i = 1:mat_size(3)
    inten_M_Inter = abs(Inter(:,:,i)).^normal_sel;
    final_density_Inter(:,:,i) = inten_M_Inter/max(max(inten_M_Inter));
...
for i = 1:mat_size(3)
    figure()
    pcolor(X,Y,final_density_Inter(:,:,i))
    colormap hot
    shading flat
    viscircles([0,locate_poten],sigr_Poten,'LineStyle','--');
    axis([-32 32 -32 32]);
    title(['time : ',sprintf('%g',time(i)),'ps'],"FontSize",20);
    saveas(gcf,sprintf('images_interference/%d.jpg',i))
    close all
end
