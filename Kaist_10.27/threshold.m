...
tfinish = 100;	% Finish time (ps)     % reservoir steady state time needed: 100 ps; +20 ps;
tp = tfinish/2;
...
RL = sqrt(  (X/ec_ra).^2 + (Y-locate_pump).^2  );
RL_non = sqrt(  (((X-locate_non_X)*cAng + (Y-locate_non_Y)*sAng)/(1.5*ec_ra)).^2 ...
                  + ( (-(X-locate_non_X)*sAng + (Y-locate_non_Y)*cAng)/(0.5)  ).^2           );
% RL_non = sqrt((X-locate_non_X).^2+(Y-locate_non_Y).^2);
...
test_Pn = Inten_Pr*exp(-(RL_non/sigr_non).^2);
% Pr(Y<locate_pump) = 0;
...
figure()
pcolor(x,y,abs(Pr)+abs(test_Pn));
% hold on
% % pcolor(x,y,abs(test_Pn));
% hold on
axis([-area_s area_s -area_s area_s])
...
A0 = 0.2;
k0=0.5;
Inter_X = M + A0*exp(-1i*k0*(X));
Inter_Y = M + A0*exp(-1i*k0*(Y));
Inter_XY = M + A0*exp(-1i*k0*(X + Y));

normal_sel = 0.6;

for i = 1:mat_size(3)
    inten_M_Inter_X = abs(Inter_X(:,:,i)).^normal_sel;
    final_density_Inter_X(:,:,i) = inten_M_Inter_X/max(max(inten_M_Inter_X));

    inten_M_Inter_Y = abs(Inter_Y(:,:,i)).^normal_sel;
    final_density_Inter_Y(:,:,i) = inten_M_Inter_Y/max(max(inten_M_Inter_Y));

    inten_M_Inter_XY = abs(Inter_XY(:,:,i)).^normal_sel;
    final_density_Inter_XY(:,:,i) = inten_M_Inter_XY/max(max(inten_M_Inter_XY));
end
...
for i = 1:mat_size(3)
    ...
    saveas(gcf,sprintf('images_intensity/%d.jpg',i))
    close all
end

for i = 1:mat_size(3)
    figure()
    pcolor(X,Y,final_density_Inter_X(:,:,i))
    colormap hot
    shading flat
    % viscircles([0,locate_poten],sigr_Poten,'LineStyle','--');
    axis([-50 50 -50 50]);
    title(['time : ',sprintf('%g',time(i)),'ps'],"FontSize",20);
    saveas(gcf,sprintf('images_interference_X/%d.jpg',i))
    close all
end

for i = 1:mat_size(3)
    figure()
    pcolor(X,Y,final_density_Inter_Y(:,:,i))
    colormap hot
    shading flat
    % viscircles([0,locate_poten],sigr_Poten,'LineStyle','--');
    axis([-50 50 -50 50]);
    title(['time : ',sprintf('%g',time(i)),'ps'],"FontSize",20);
    saveas(gcf,sprintf('images_interference_Y/%d.jpg',i))
    close all
end

for i = 1:mat_size(3)
    figure()
    pcolor(X,Y,final_density_Inter_XY(:,:,i))
    colormap hot
    shading flat
    % viscircles([0,locate_poten],sigr_Poten,'LineStyle','--');
    axis([-50 50 -50 50]);
    title(['time : ',sprintf('%g',time(i)),'ps'],"FontSize",20);
    saveas(gcf,sprintf('images_interference_XY/%d.jpg',i))
    close all
end

for i = 1:mat_size(3)
    figure()
    pcolor(X,Y,angle(M(:,:,i))  )
    colormap gray
    shading flat
    % viscircles([0,locate_poten],sigr_Poten,'LineStyle','--');
    axis([-50 50 -50 50]);
    title(['time : ',sprintf('%g',time(i)),'ps'],"FontSize",20);
    saveas(gcf,sprintf('images_phase/%d.jpg',i))
    close all
end
