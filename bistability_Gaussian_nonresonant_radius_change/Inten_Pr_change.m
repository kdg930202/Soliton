...
function [l,r,Pr_l,Pr_r] = Inten_Pr_change(Inten_Pr)
% print(sigr_non)
tic
...
tfinish = 1400;	% Finish time (ps)     % reservoir steady state time needed: 100 ps; +20 ps;
tp = tfinish/2;
tn = 1; %ps;
...
nuc=32; % Number of Unit Cell in one direction from 0-end
...
% Inten_Pr = 0.05*20;
% Inten_Pr = 1; %when tn=500
% Inten_Pr = 0.002; %when tn=1
% Inten_Pr = 500;
% Inten_Pr = 0;
% Inten_Pr = 0.05*100;
sigr = 6; % size
...
RL_non = sqrt(X.^2+(Y+locate_poten).^2);
...
para = struct('hbar',hbar,'sstep',sstep,'alpha',alphaNL,'tauP',tauP, ...
    'Pr',Pr,'Pn',Pn,'omega',omega,'tp',tp,'Gamma',Gamma,'EC',EC,'Gin',Gin,'beta',beta,'tauR',tauR,'Poten',Poten,'tn',tn);
...
% 
time_l = time(1:find(time==tp));
time_r = flip(time(1:find(time==tp)-1));
l = result(1:find(time==tp));
r = result(find(time==tp)+1:end);
